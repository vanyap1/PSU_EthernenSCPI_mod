...
struct usart_async_descriptor DEBUG_SERIAL;
...
uint8_t rxBytesCount = 0;
volatile uint8_t serialReceiving = 0;
volatile uint8_t serialTransmitting = 0;
volatile uint8_t serialComplete = 0;
volatile uint8_t serialReceivedBytesCounter = 0;
volatile uint8_t serialBufferOverflow = 0;
...
void gpio_init(void){
	...
	gpio_set_pin_direction(REM, GPIO_DIRECTION_OUT);
	gpio_set_pin_function(REM, GPIO_PIN_FUNCTION_OFF);
...
//DEBUG_SERIAL.rx.write_index;
//DEBUG_SERIAL.rx.buf
//DEBUG_SERIAL.rx.read_index
...
void DEBUG_Serial_init(void) {
	...
	//(A)1234567890abcdefghkl(B)1234567890abcdefghkl(C)1234567890abcdefghkl(D)1234567890abcdefghkl
